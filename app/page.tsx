...
import { Upload, Download, X, BarChart3, Heart, FileText, Users, HelpCircle, Trash2, Copy, Image as ImageIcon } from 'lucide-react';
...
type AppTab = 'upload' | 'howto' | 'filters' | 'squad' | 'compare' | 'screenshot';
...
export default function FMValueScoutV5() {
  ...
  const calculateValueScore = (row: PlayerRawData, position: string, league: string) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        const parsedPlayers: Player[] = results.data
          .map((rowData, index: number) => {
            const row = rowData as PlayerRawData;
            ...
            return {
              ...
              age,
              position: group,
              league: String(league || 'Unknown League'),
              valueScore: score,
  ...
  const getStatColor = (key: string, value: string | number | undefined, position: string) => {
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 flex gap-8">
        ...
        <div className="flex-1">
          <div className="flex border-b border-violet-900/50 mb-8 overflow-x-auto">
            {(['upload', 'howto', 'filters', 'squad', 'compare', 'screenshot'] as AppTab[]).map((tab) => (
          ...
          {activeTab === 'howto' && (
            <div className="bg-zinc-900/80 border border-violet-900/50 rounded-3xl p-10 space-y-8 text-zinc-300">
              <div>
                <h2 className="text-3xl font-bold mb-4 text-white">How to Use</h2>
                <p>Use CSV upload for best accuracy. Screenshot OCR works as a fallback and may need manual verification.</p>
              </div>
              <div>
                <h3 className="text-xl font-semibold text-emerald-400 mb-2">Recommended flow</h3>
                <p>Export player data from FM, upload CSV, sort by Value Score, then add targets to shortlist.</p>
              </div>
            </div>
          )}
          {activeTab === 'filters' && (
            <div className="bg-zinc-900/80 border border-violet-900/50 rounded-3xl p-10">
              <h2 className="text-3xl font-bold mb-6">Export Filters</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(recommendedColumns).map(([position, cols]) => (
                  <div key={position} className="bg-zinc-800 rounded-2xl p-5">
                    <h3 className="font-semibold text-violet-300 mb-3">{position}</h3>
          ...
          {activeTab === 'squad' && (
            <div className="bg-zinc-900/80 border border-violet-900/50 rounded-3xl p-10">
              ...
              <div
                className="bg-zinc-950 border-2 border-dashed border-violet-700 h-64 rounded-3xl flex items-center justify-center text-zinc-400"
                onDrop={handleDrop}
                onDragOver={(e) => e.preventDefault()}
              >
                Drag players from shortlist into this area
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 text-sm">
                <div className="bg-zinc-800 rounded-2xl p-4">Avg Score: <span className="text-emerald-400 font-bold">{squadStats.avgScore}</span></div>
                <div className="bg-zinc-800 rounded-2xl p-4">Total Wage: <span className="text-violet-400 font-bold">£{squadStats.totalWage}k</span></div>
                <div className="bg-zinc-800 rounded-2xl p-4">Avg Age: <span className="text-purple-400 font-bold">{squadStats.avgAge}</span></div>
                <div className="bg-zinc-800 rounded-2xl p-4">Hidden Gems: <span className="text-amber-400 font-bold">{squadStats.gems}</span></div>
              </div>
            </div>
          )}
          ...
          <div className="bg-zinc-900/80 border border-violet-900/50 rounded-3xl p-6 sticky top-24">
            ...
            <div className="space-y-3">
              <a href="https://twitter.com/JakeSummersFM" target="_blank" className="flex items-center gap-3 p-3 bg-zinc-800 hover:bg-zinc-700 rounded-2xl transition">
                <span className="w-5 h-5 text-sky-400 font-bold">X</span> @JakeSummersFM
              </a>
              <a href="https://www.twitch.tv/jakesummersfm" target="_blank" className="flex items-center gap-3 p-3 bg-zinc-800 hover:bg-zinc-700 rounded-2xl transition">
                <span className="w-5 h-5 text-purple-400 font-bold">TV</span> Twitch
              </a>
